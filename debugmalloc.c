...
static void debugmalloc_log(char const *format, ...) {
    ...
    if (strcmp(instance->logfile, "") != 0) {
        ...
        if (f == NULL) {
            f = stderr;
            fprintf(stderr, "debugmalloc: cannot open file %s!\n", instance->logfile);
...
static void debugmalloc_dump_elem(DebugmallocElem const *elem) {
    ...
    debugmalloc_log("  BLOCK: %p, %u bytes, canary: %s\n"
                    "    allocated at: %s:%u, %s(%s)\n",
                       elem->user_mem, (unsigned) elem->size, canary_ok ? "ok" : "**smashed**",
                       elem->file, elem->line, elem->func, elem->expr);
    ...
    debugmalloc_log("    allocated block:\n");
...
void debugmalloc_dump(void) {
    DebugmallocData *instance = debugmalloc_singleton();
    debugmalloc_log("** DEBUGMALLOC DUMP BEGIN ************************************\n");
...
void *debugmalloc_malloc_full(size_t size, char const *func, char const *expr, char const *file, unsigned line, bool zero) {
    ...
    if (real_mem == NULL) {
        debugmalloc_log("debugmalloc: %s @ %s:%u: unable to allocate %u bytes\n", func, file, line, (unsigned) size);
        return NULL;
    }
...
void debugmalloc_free_full(void *mem, char const *func, char const *file, unsigned line) {
    ...
    if (deleted == NULL) {
        debugmalloc_log("debugmalloc: %s @ %s:%u: block is not allocated\n", func, file, line);
        abort();
    }

    /* check canary and then free memory */
    if (!debugmalloc_canary_ok(deleted)) {
        debugmalloc_log("debugmalloc: %s @ %s:%u: buffer overrun in block %p\n", func, file, line, mem);
...
void *debugmalloc_realloc_full(void *oldmem, size_t newsize, char const *func, char const *expr, char const *file, unsigned line) {
    ...
    if (entry == NULL) {
        debugmalloc_log("debugmalloc: %s @ %s:%u: cannot realloc block that is not allocated\n", func, file, line);
        abort();
    }
    ...
    if (newmem == NULL) {
        debugmalloc_log("debugmalloc: %s @ %s:%u: cannot allocate new block for realloc\n", func, file, line);
...
static DebugmallocData * debugmalloc_create(void) {
    ...
    if (debugmalloc_canary_size % 16 != 0) {
        debugmalloc_log("debugmalloc: size of canary must be multiple of 16\n");
        abort();
    }
    if (debugmalloc_canary_char == 0) {
        debugmalloc_log("debugmalloc: canary byte should not be zero\n");
        abort();
    }
    ...
    if (instance == NULL) {
        debugmalloc_log("debugmalloc: cannot initialize checked memory\n");
        abort();
    }
